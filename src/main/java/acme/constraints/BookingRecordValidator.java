...
public class BookingRecordValidator extends AbstractValidator<ValidBookingRecord, BookingRecord> {
	...
	public boolean isValid(final BookingRecord bookingRecord, final ConstraintValidatorContext context) {
		...
		else if (SpringHelper.getBean(BookingRepository.class).findPassengersByBookingId(bookingRecord.getId()).contains(bookingRecord.getPassenger()))
			super.state(context, false, "passenger", "acme.validation.passenger.already-on-board");
		else if (!SpringHelper.getBean(BookingRepository.class).findUndraftedPassengerByCustomerId(bookingRecord.getBooking().getCustomer().getId()).contains(bookingRecord.getPassenger()))
